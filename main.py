...
policy_net = DQN(len(preprocessor.orders), attribute_count, len(path_env.actions), device).to(device)

optimizer = optim.Adam(policy_net.parameters(), lr=config.LEARNING_RATE)
path_agent = PathAgent(policy_net, device, len(preprocessor.orders), attribute_count,
                       len(path_env.actions), optimizer)
...
for i_episode in range(config.TRAINING_EPISODE):
    ...
    for t in count():
        ...
        action = path_agent.select_action(state)
        total_step += 1
        _, reward, done = path_env.step(action.item())
        reward = torch.tensor([reward], device=device)

        next_state = path_env.get_current_state()
        path_agent.memory.push(state, action, next_state, reward)
        # Move to the next state
        state = next_state

        # Perform one step of the optimization (on the policy network)
        loss = path_agent.optimize_model()
        ...
        if total_step % config.EVAL_INTERVAL == 0:
            avg_return = path_agent.test_agent(test_env, len(preprocessor.job_info_list),
                                               preprocessor.equipments_list, False)
            if best_return < avg_return:
                best_return = avg_return
                tolerance_count = 0
                # Store good results
                path_agent.save_best_observation(test_env, preprocessor.equipments_list, preprocessor.job_info_list)
                # Save model
                torch.save(path_agent.policy_net.state_dict(), config.MODEL_FILE)
            else:
                tolerance_count += 1
        ...
        if total_step % 5000 == 0:
            ...
            path_agent.test_agent(test_env, len(preprocessor.job_info_list), preprocessor.equipments_list)
        ...
        if done:
            if all([ob.is_done for ob in path_env.job_info.observation]):
                path_agent.save_best_observation(test_env, preprocessor.equipments_list, preprocessor.job_info_list)
                finish_schedule_result = path_agent.best_observation[-1]
                ...
                torch.save(path_agent.policy_net.state_dict(), config.MODEL_FILE + '_all_job_done')
                sys.exit()
            break

    # Update the target network, copying all weights and biases in DQN
    if i_episode % config.TARGET_UPDATE_PERIOD == 0:
        path_agent.update_target_net()
