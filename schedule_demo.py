...
convs = tf.keras.layers.Conv1D(filters=16, kernel_size=2,activation='relu')
q_net = sequential.Sequential([convs]+[tf.keras.layers.Flatten()] + dense_layers + [q_values_layer])

optimizer = tf.keras.optimizers.Adam(learning_rate=learning_rate)
# optimizer = tf.keras.optimizers.RMSprop(
#     learning_rate=learning_rate, rho=0.9, momentum=0.9, epsilon=1e-07, centered=True, name='RMSprop')
# optimizer = tfa.optimizers.Lookahead(tfa.optimizers.RectifiedAdam(learning_rate=learning_rate))
